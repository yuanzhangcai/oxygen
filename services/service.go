...
import (
	...
	"github.com/yuanzhangcai/oxygen/middleware"
...
func CreateRouters(router *gin.Engine) {
	...
	// 设置获取版本信息接口路由
	HandleAll(router, "/version", []string{http.MethodGet, http.MethodPost}, HandleMain(&controllers.Controller{}, "Version"))

	// // 主入口
	// HandleAll(router, "/main", []string{http.MethodGet, http.MethodPost}, HandleMain(&controllers.EngineCtl{}, "Main"))

	// // 强行清除本地缓存
	// HandleAll(router, "/main/clear_cache", []string{http.MethodGet, http.MethodPost}, HandleMain(&controllers.EngineCtl{}, "ClearCache"))
}

type handleFun func(httpMethod, relativePath string, handlers ...gin.HandlerFunc) gin.IRoutes

// HandleAll 批量设置路由
func HandleAll(r interface{}, relativePath string, httpMethods []string, handlers ...gin.HandlerFunc) {
	...
	for _, httpMethod := range httpMethods {
		handle(httpMethod, relativePath, handlers...)
	}
}
...
// 	serverName := config.Get("micro", "server_name").String("oxygen.zacyuan.com") // 微务服名称
